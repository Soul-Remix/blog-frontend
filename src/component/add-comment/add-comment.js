...
const AddComment = ({ length, showComment }) => {
  return (
    <p className="no-comment">
      {length === 0 ? 'No comment yet' : length + ' ' + 'Comments'}
      <button className="add-comment-btn" onClick={showComment}>
        <svg className="symbol-add" viewBox="0 0 512 512">
